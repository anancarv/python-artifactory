...
FEDERATED_REPOSITORY = RemoteRepository(key="test_federated_repository", url="http://test-url.com")
FEDERATED_REPOSITORY_RESPONSE = FederatedRepositoryResponse(key="test_remote_repository", url="http://test-url.com")
...
@responses.activate
def test_create_local_repository_fail_if_repository_already_exists(
    mocker,
):
    responses.add(
        responses.GET,
...
@responses.activate
def test_create_virtual_repository_fail_if_repository_already_exists(
    mocker,
):
    responses.add(
        responses.GET,
...
@responses.activate
def test_create_remote_repository_fail_if_repository_already_exists(
    mocker,
):
    responses.add(
        responses.GET,
...
@responses.activate
def test_create_virtual_repository_repo_success(mocker):
...
def test_create_federated_repository_success(mocker):
    ...
    artifactory_repo = ArtifactoryRepository(AuthModel(url=URL, auth=AUTH))
    mocker.spy(artifactory_repo, "get_repo")
    remote_repo = artifactory_repo.create_repo(FEDERATED_REPOSITORY)
...
@responses.activate
def test_get_local_repository_success():
    responses.add(
        responses.GET,
        f"{URL}/api/repositories/{LOCAL_REPOSITORY.key}",
        json=LOCAL_REPOSITORY_RESPONSE.model_dump(),
...
@responses.activate
def test_get_remote_repository_success():
    responses.add(
        responses.GET,
        f"{URL}/api/repositories/{REMOTE_REPOSITORY.key}",
        json=REMOTE_REPOSITORY_RESPONSE.model_dump(),
...
@responses.activate
def test_update_virtual_repository_success():
    responses.add(
        responses.GET,
